...
const PersonalWebsite = {
  ...
  schedule: {
    ...
    /**
     * Initialize real-time synchronization with Firebase for cross-computer collaboration
     */
    initializeRealTimeSync: function() {
      ...
      try {
        ...
        const usersRef = window.firebaseDB.ref('activeUsers');
        
        // Generate unique computer/session ID
        this.computerId = 'computer_' + Math.random().toString(36).substr(2, 9);
        this.computerName = this.generateComputerName();
        
        // Register this computer as active
        this.registerComputer();
        
        // Listen for real-time schedule updates from other computers
        scheduleRef.on('value', (snapshot) => {
          const data = snapshot.val();
          if (data && !this.isUpdatingFirebase) {
            console.log('📡 Received update from another computer');
            this.scheduleData = data;
            this.regenerateSchedule();
            this.showCrossComputerNotification('📅 Schedule updated by another computer!');
            this.highlightChangedCells();
          }
        });
        
        // Listen for other computers connecting/disconnecting
        usersRef.on('value', (snapshot) => {
          const users = snapshot.val() || {};
          this.updateConnectedComputers(users);
        });
        ...
        scheduleRef.once('value', (snapshot) => {
          if (snapshot.exists()) {
            ...
            this.regenerateSchedule();
            console.log('📊 Schedule loaded from cloud');
          } else {
            // Upload default schedule to Firebase
            this.syncToFirebase();
            console.log('📤 Default schedule uploaded to cloud');
          }
        });
        
        this.isConnected = true;
        this.updateConnectionStatus(true);
        console.log('🌐 Cross-computer real-time sync initialized');
    ...
    /**
     * Register this computer as active user
     */
    registerComputer: function() {
      if (!this.isConnected || !window.firebaseDB) return;
      
      const computerRef = window.firebaseDB.ref(`activeUsers/${this.computerId}`);
      ...
      // Remove computer info when disconnecting
      computerRef.onDisconnect().remove();
      
      // Monitor connection status
      window.firebaseDB.ref('.info/connected').on('value', (snapshot) => {
        if (snapshot.val()) {
          computerRef.child('online').set(true);
          computerRef.child('lastSeen').set(firebase.database.ServerValue.TIMESTAMP);
        }
      });
    },
    
    /**
     * Generate friendly computer name
     */
    generateComputerName: function() {
      const adjectives = ['Blue', 'Red', 'Green', 'Purple', 'Orange', 'Pink', 'Yellow', 'Cyan'];
      const nouns = ['Computer', 'Laptop', 'Desktop', 'Device', 'Machine', 'Workstation'];
     ...
     */
    updateConnectedComputers: function(users) {
      const computerCount = Object.keys(users).length;
      const statusElement = document.getElementById('connectionStatus');
      
      if (statusElement && this.isConnected) {
        const indicator = statusElement.querySelector('.status-indicator');
        if (indicator) {
          indicator.innerHTML = `🌐 ${computerCount} computer${computerCount !== 1 ? 's' : ''} connected`;
          
          // Show computer names
          const computerNames = Object.values(users).map(user => user.name).join(', ');
          indicator.title = `Connected: ${computerNames}`;
        }
      }
    },
    
    /**
     * Show cross-computer notification
     */
    showCrossComputerNotification: function(message) {
      this.showNotification(message, 'info');
      
      // Add visual pulse effect to indicate cross-computer update
      document.body.style.animation = 'crossComputerPulse 0.5s ease-in-out';
      setTimeout(() => {
    ...
    /**
     * Highlight cells that changed from other computers
     */
    highlightChangedCells: function() {
      const cells = document.querySelectorAll('.schedule-cell');
      cells.forEach(cell => {
        cell.classList.add('cross-computer-update');
        setTimeout(() => {
          cell.classList.remove('cross-computer-update');
        }, 2000);
      });
    },
    
    /**
     * Set computer editing status
     */
    setComputerEditingStatus: function(isEditing) {
      if (!this.isConnected || !window.firebaseDB) return;
      
      window.firebaseDB.ref(`activeUsers/${this.computerId}/editing`).set(isEditing);
    },
    ...
    /**
     * Enter edit mode with cross-computer notification
     */
    enterEditMode: function() {
      this.isEditMode = true;
      this.originalScheduleData = JSON.parse(JSON.stringify(this.scheduleData));
      this.setComputerEditingStatus(true);
      ...
      // Show notification
      this.showNotification('✏️ Edit mode enabled. Changes will sync to all computers instantly!', 'success');
    },
    ...
    exitEditMode: function() {
      ...
      this.setComputerEditingStatus(false);
    ...
    /**
     * Save modal entry with instant cross-computer sync
     */
    saveModalEntry: function() {
      ...
      if (type === 'empty' || activity === '') {
        ...
        this.scheduleData[day][time] = {
          ...
          modifiedBy: this.computerId,
          computerName: this.computerName
        };
      }
      
      // Sync to Firebase immediately for cross-computer updates
      this.syncToFirebase();
      ...
      this.showNotification('🌐 Change synced to all computers instantly!', 'success');
    },
